...
$pegawai = query("SELECT * FROM pegawai WHERE  is_delete=1 and id_pegawai!=11 ORDER BY id_pegawai");


if (isset($_POST["konfirmasi"])) {
  // var_dump($_POST);die;
  if (konfirmasiDataPegawai($_POST) > 0) {
    echo "
       <script>
...
<div class="container-fluid">
  ...
  <div class="card shadow mb-2">
              ...
              <tr>
                ...
                <td class="text-center">
                  <form method="POST">
                    ...
                    <button type="button" id="status" name="status" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#modalStatus<?= $row['id_pegawai']; ?>">
                    ...
                    <input type="hidden" name="id_pegawai" value="<?= $row['id_pegawai']; ?>">
                        ...
                        <button type="submit" name="hapus" class="btn btn-danger btn-sm" onclick="return confirm('yakin hapus <?= $row['nm_pegawai'] ?>?');"><i class="fas fa-trash-alt"></i> Delete</button>
                     <?php  }
    ...
    <!-- Modal Edit Data -->
    <?php foreach ($pegawai as $row) : ?>
      <div class="modal fade" id="modalStatus<?= $row['id_pegawai'] ?>" tabindex="-2" role="dialog" aria-labelledby="modalStatusDataTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
          <div class="modal-content">
            <form method="post" enctype="multipart/form-data">
              <div class="modal-header modal-bg" back>
                <h5 class="modal-title modal-text" id="modalEditDataTitle">Form Status Akun Pegawai</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
              <div class="modal-body">
                <form>
                  ...
                  <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                    <button type="submit" name="status_edit" class="btn btn-primary">Update</button>
                  </div>
                </form>
              </div>
            </form>
