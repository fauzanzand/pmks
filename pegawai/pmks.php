...
// }
  ...
  $kk=query("SELECT a.*, p.nm_pegawai, b.id_kec,b.nm_kec,b.is_delete,c.id_desa,c.nm_desa,c.is_delete FROM kk a 
               INNER JOIN kecamatan b USING (id_kec)
               INNER JOIN desa c USING (id_desa)
               INNER JOIN pegawai p ON a.creator=p.id_pegawai
               WHERE a.is_delete=1 AND b.is_delete=1 AND c.is_delete=1 AND c.id_desa='$id_desa'
                  ...
                  <?php foreach ($kk as $row):
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td class="text-left"><?=$row['nm_pegawai']?></td>
                        <td class="text-left"><?=$status?></td>
                        <td class="text-center">
                          <form method="POST">
                            <?php
                            if($row['nm_pegawai'] == $_SESSION['nama'])
                              { ?>
                            ...
                            <button type="button" id="pmks" name="pmks" class="btn btn-info btn-sm mt-2" data-toggle="modal" data-target="#modalPmks<?= $row['id_kk']; ?>">
                            <i class="fas fa-edit"></i> (+)PMKS</button>
                            <?php }
                            else{
                              ?>
                                <button type="button" id="edit" name="edit" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#modalEdit<?= $row['id_kk']; ?>">
                            ...
                            <button type="submit" name="hapus" class="btn btn-danger btn-sm" onclick="return confirm('yakin hapus <?=$row['nm_kpl'] ?>?');">
                            <i class="fas fa-trash-alt"></i></button><br>
                            <button type="button" id="pmks" name="pmks" class="btn btn-info btn-sm mt-2 mb-2" data-toggle="modal" data-target="#modalPmks<?= $row['id_kk']; ?>">
                            <i class="fas fa-edit"></i> (+)PMKS</button><br>
                            <button type="submit" name="setuju" class="btn btn-success btn-sm" onclick="return confirm('yakin setujui <?=$row['nm_kpl'] ?>?');">
                            <i class="fas fa-check"></i></button>
                            <button type="submit" name="tolak" class="btn btn-danger btn-xs pull-right" onclick="return confirm('yakin tolak <?=$row['nm_kpl'] ?>?');">
                        ...
                        <div class="modal-body">
                          <form>
                            ...
                            <div class="form-group">
                              ...
                              <input type="text" class="form-control mt-1" id="tgl_lhr_kpl" name="tgl_lhr_kpl"  required>
                            </div>
                       ...
                       <div class="modal-body">
                        <form>
                           ...
                           <div class="form-group">
                            ...
                            <div class="form-group">
                              <label for="tgl_lhr_kpl" class="col-form-label">Tanggal Lahir Kepala Keluarga:</label>
                              <input type="text" class="form-control mt-1" id="tgl_lhr_kpl" name="tgl_lhr_kpl" value="<?=$row['tgl_lhr_kpl']?>" required>
                            </div>
                       ...
                       <div class="modal-body">
                        <form>
                            ...
                            <div class="form-group">
                              ...
                              <input type="text" class="form-control mt-1" id="tgl_lhr_pmks" name="tgl_lhr_pmks" required>
                            </div> 
