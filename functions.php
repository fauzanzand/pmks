...
function register($data)
{
	...
	$id_desa    = htmlspecialchars($data["id_desa"]);
	...
	$query = "INSERT INTO pegawai (nm_pegawai, nip, no_telepon, id_desa, username, password, role, is_delete, row_edit, time_input) VAlUES ('$nama','$nip','$no_tlp','$id_desa','$username','$password','$role',1,0,'$time_input')";
...
function tambahDataKk($data)
{
	...
	if($id_desa == $_SESSION['id_desa']){
		$status = 1;
	}else{
		$status = 0;
	}
	//cek no kk sudah ada atau belum
	$result = mysqli_query($koneksi, "SELECT nik_kpl FROM kk WHERE nik_kpl = '$nik_kpl' AND is_delete=1 ");

	if (mysqli_fetch_assoc($result)) {
		echo "<script>
 				alert('No NIK sudah Ada!')
 			  </script>";
		return false;
	}
	...
	$query = "INSERT INTO kk VAlUES ('','$id_kec','$id_desa','$no_kk','$nik_kpl',
 									 ...
 									 '$pendidikan_kpl','$status','$is_delete','$row_edit','$creator','$time_input')";
	mysqli_query($koneksi, $query);
	return mysqli_affected_rows($koneksi);
}
...
function setujuiKK($data)
{
	include('config.php');

	$id_kk = $data["id_kk"];
	
	$query 	= "UPDATE kk SET status = 1 WHERE id_kk='$id_kk'";
	mysqli_query($koneksi, $query);

	return mysqli_affected_rows($koneksi);
}

function tolakKK($data)
{
	include('config.php');

	$id_kk = $data["id_kk"];

	$query 	= "UPDATE kk SET status = 0 WHERE id_kk='$id_kk'";
	mysqli_query($koneksi, $query);
...
function uploadDataKk($data)
{
	...
	$no_kk2				= $data->val($i,1);
	$nik_kpl2			= $data->val($i,2);
	$nm_kpl2			= $data->val($i,3);
	$tgl_lhr_kpl2		= $data->val($i,4);
	$alamat_kpl2		= $data->val($i,5);
	$pekerjaan_kpl2		= $data->val($i,6);
	$pendidikan_kpl2	= $data->val($i,7);
	$is_delete2			= $data["is_delete2"];
	$row_edit2 			= $data["row_edit2"];
	$creator2			= $data["id_pegawai2"];
	$time_input2 		= $data["time_input2"];
...
function tambahDataPmks($data)
{
	...
	$result = mysqli_query($koneksi, "SELECT nik_pmks FROM pmks WHERE nik_pmks = '$nik_pmks' AND is_delete=1 AND nik_pmks!='Tidak Ada'");
...
function editDataStatus($data)
{
	...
	$query = "UPDATE pegawai SET status='$role' WHERE id_pegawai='$id_pegawai'";

	mysqli_query($koneksi, $query);
	return mysqli_affected_rows($koneksi);
}

function konfirmasiDataPegawai($data)
{
	include('config.php');
	$id_pegawai = $data["id_pegawai"];
...
function editDataPegawai($data)
{
 ...
 // 			  </script>";
	...
	$query = "UPDATE pegawai SET 
 				   	 ...
 				   	 no_telepon='$no_telepon', id_desa='$id_desa',
...
function ubahpass($data)
{
	...
	$password_a 	 = password_hash($password, PASSWORD_DEFAULT);
	...
	$ambil_p = mysqli_query($koneksi, "SELECT password FROM pegawai WHERE id_pegawai='$id_pegawai'");
	$pwd = mysqli_fetch_array($ambil_p);
	$password_s = $pwd['password'];

	if( mysqli_num_rows($result) == 1 ) {
	    	
	    	//cek password lama

			if(password_verify($password, $password_s)==false) {
			 	echo "<script>
...
function statusDataPmks($data)
{
	include('config.php');
// var_dump($data);die;
	$id_pmks = $data["id_pmks"];
	$hsl_survei = $data["hsl_survei"];
	// echo $id_pmks."-".$hsl_survei;die;
	$query 	= "UPDATE pmks SET hsl_survei = '$hsl_survei' WHERE id_pmks='$id_pmks'";
	mysqli_query($koneksi, $query);
