...
if (isset($_POST["login"])) {
  ...
  $result = mysqli_query($koneksi, "SELECT pegawai.*, desa.id_desa, desa.id_kec FROM pegawai INNER JOIN desa ON pegawai.id_desa=desa.id_desa WHERE username = '$username'");
  // cek username
  ...
  if (mysqli_num_rows($result) == 1) {
   ...
   if ($row["status"] == 1){
    ...
    if (password_verify($password, $row["password"]) && $row["is_delete"]==1) {
      if ($row['role'] == Admin) {
        $_SESSION["admin"] = true;
        $_SESSION["id_kec"] = $row['id_kec'];
        $_SESSION["id_desa"] = $row['id_desa'];
              ...
              <div class="col-lg-12">
                <div class="p-5">
                  <div class="text-center">
                    ...
                    <hr>
                    <h1 class="h4 text-gray-900 mb-4">PANDATAAN PMKS ONLINE</h1>
                  </div>

                  <?php
                  if (isset($error)) : ?>
                    <div class="wrap-input100 validate-input m-b-10 alert alert-danger text-center" role="alert">
                     Username atau Password Salah <p>Coba Lagi!</p>
                    </div>
                  <?php endif; ?>
                   <?php
                  if (isset($error2)) : ?>
                  ...
                  <form class="user" action="" method="post">
                    <div class="form-group">
                      <input type="text" name="username" id="username" placeholder="Username" class="form-control form-control-user" aria-describedby="emailHelp" placeholder="Enter Username..."required>
                    </div>
                    <div class="form-group">
